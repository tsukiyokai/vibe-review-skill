...
#### 规则 BUILD-05: CMake OP_NAME/target名复制粘贴错误
...
#### 规则 BUILD-07: auto-sync编译选项与kernel代码冲突
...
算子仓库特有的高频模式。根因是多layout(BNSD/BSND/TND/NTD) x 多模式(GQA/MHA/MQA) x 多量化(FP8/INT8/Perblock) x 多sparse mode的组合空间爆炸，开发者通常只验证常见组合。dev仓(~75条, 14.2%)进一步印证此类缺陷的普遍性，OP_CHECK_IF宏的语义反转和逻辑运算符混淆是dev仓新增的高频子模式。
...
缺陷描述: 多维枚举(GQA/MHA x BNSD/TND x 量化/非量化 x 对齐/非对齐)的组合空间中遗漏"边角组合"。if-else只处理了常见路径，特殊组合穿透到默认分支产生错误。dev仓中新增枚举值(sparseMode、quantMode、Layout等)或运行模式后，代码中多处if/switch分支未覆盖新值的模式约20条。
...
#### 规则 LOGIC-05: OP_CHECK_IF/OP_CHECK条件语义反转
...
#### 规则 LOGIC-06: 逻辑运算符OR/AND混淆导致恒真/恒假
...
#### 规则 LOGIC-07: Layout专属逻辑在错误Layout下执行
...
#### 规则 BOUND-05: GM访问前缺边界检查(check-before-access)
...
#### 规则 API-05: return false在graphStatus函数中(语义反转)
...
#### 规则 STATE-04: 条件分支遗漏赋值(if赋值else未赋值)
...
缺陷描述: workspace大小、buffer对齐、struct类型均出现tiling/kernel两侧独立计算不匹配。根因是两侧代码物理分离（op_host/ vs op_kernel/，不同编译单元）缺乏编译期一致性约束。dev仓中tilingKey按位组装(host) vs 模板参数解析(kernel)约20条、workspace内存布局(host offset) vs kernel读取offset约8条、宏参数顺序 vs 宏定义约5条，进一步印证此系统性风险。
