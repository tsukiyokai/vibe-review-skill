...
# Appendix: Referenced Abseil C++ Tips

The following Abseil "Tip of the Week" articles are referenced by the Google C++ Style Guide.

---

## TotW #42: Prefer Factory Functions to Initializer Methods

*Originally posted as totw/42 on 2013-05-10, by Geoffrey Romer. Revised 2017-12-21.*

Source: <https://abseil.io/tips/42>

### Problem with Initializer Methods

In environments without exceptions (like Google's codebase), constructors must succeed since they cannot report failure. When initialization logic might fail, developers often use initializer methods that signal failure through return values.

However, this approach creates problems: "This design commits you to maintaining a class with at least two distinct user-visible states, and often three: initialized, uninitialized, and initialization-failed."

Users may call other methods before initialization completes or after it fails. This violates intended usage patterns but becomes difficult to prevent.

### Solution: Factory Functions

Factory functions create and fully initialize objects before returning them, using pointers or `absl::optional` to indicate failure:
...
Factory functions expose only fully-initialized objects while signaling failure like initializer methods. They also enable returning different subclass implementations dynamically, allowing implementation swaps without updating user code.

### Limitations

Factory functions work best for heap-allocated objects, not stack-based value-like classes (which typically don't need complex initialization anyway). They also cannot initialize base classes in derived constructors, though public APIs can still use factories with protected initializer methods as needed.

---

## TotW #109: Meaningful `const` in Function Declarations

Source: <https://abseil.io/tips/109>

### Key Distinction: Declarations vs. Definitions

Function *declarations* (signature only) and *definitions* (with function body) treat `const` differently. Top-level `const` qualifiers on parameters are ignored in declarations but affect the parameter type within the function body in definitions.

### When `const` is Meaningful

`const` qualifications "buried within a parameter type specification" are significant:

```cpp
void F(const int* x);                  // pointer to const int
void F(const int& x);                  // reference to const int
void F(std::unique_ptr<const int> x);  // unique_ptr to const int
```

These are valid overloads because the `const` is part of the *pointed-to* or *referenced* type, not a top-level qualifier on the parameter itself.

### When `const` is Meaningless

Top-level `const` qualifiers are ignored:

```cpp
void F(const int x);    // treated as F(int)
void F(int* const x);   // treated as F(int*)
```

### Recommended Guidelines

1. Avoid top-level `const` in non-definition declarations; it is visual noise.
2. In function definitions, use top-level `const` at team discretion, applying the same reasoning as for local variable constness.

---

## TotW #116: Keeping References on Arguments

Source: <https://abseil.io/tips/116>

### Key Problem

Storing const references to function arguments in class members can create dangling references if those arguments are temporaries:
...
void Func() {
  Foo foo("something");
  LOG(INFO) << foo.content();  // BOOM! Dangling reference
}
```

The temporary string is destroyed at the end of the initialization line, leaving `content_` as a reference to deleted memory.

### Recommended Solution: Use Pointers

Replace const reference parameters with const pointers when storing references:
...
class Foo {
 public:
  ...
  const std::string& content() const { return *content_; }
 private:
  const std::string* const content_;
};
```

This prevents binding to temporaries — the code fails to compile rather than creating undefined behavior.

### Key Takeaway

It is still OK to pass an argument by const reference to a constructor if the argument is copied, or just used in the constructor and no reference to it is kept in the constructed object. In other cases, consider passing arguments by pointers.

---

## TotW #130: Namespace Naming

*Originally posted February 17, 2017, by Titus Winters.*

Source: <https://abseil.io/tips/130>

### Core Problem

The article challenges the practice of structuring C++ namespaces to match package hierarchies — a pattern inherited from Java conventions. C++'s name lookup mechanism creates unexpected collision risks that Java doesn't face.

### Key Difference: Name Lookup Mechanics

Unlike Java's explicit import requirements, C++ performs unqualified name lookup through expanding scopes. When code references `Baz` without qualification, the compiler searches progressively outward: local scope, containing namespace, parent namespaces, then global scope. This creates vulnerability to "name collision at a distance."

### The Collision Risk

In deeply nested packages (like `::division::section::team::subteam::project`), an unqualified reference to `std::unique_ptr` could accidentally resolve to any matching namespace in the hierarchy. The problem intensifies with common names like `util`, `testing`, or `pipeline`.

### Best Practices

- Maintain a namespace database for uniqueness verification.
- Introduce new namespaces as top-level identifiers.
- Never create sub-namespaces matching well-known top-levels.
- Use fully-qualified names in namespace aliases and using-declarations.
- Shallow namespaces provide better protection than organizational nesting benefits.

---

## TotW #140: Constants: Safe Idioms

*Originally posted December 8, 2017, by Matt Armstrong.*

Source: <https://abseil.io/tips/140>

### Definition of a C++ Constant

A constant is "a variable or function that always evaluates to the same value," with three key components:

1. **The value** — remains unchanged (e.g., five is always five).
2. **The object** — has a value at each point in time; mutation is disallowed.
3. **The name** — named constants are more useful than bare literals.

### Safe Idioms for Header Files

#### Inline constexpr Variable (C++17+)
...
All `constexpr` functions are implicitly `inline`, eliminating linkage concerns.
...
### Why Header File Constants Are Risky

At namespace scope, both `const` and `constexpr` objects implicitly have internal linkage. Each translation unit including the header gets its own "copy" at a different address, causing:

1. Bugs — code using constant addresses suffers undefined behavior.
2. Bloat — each translation unit maintains separate copies.

Example bug (address comparison):
...
void DoSomething(const char* value) {
  if (value == kSpecial) {  // BUG: different addresses per translation unit
...
**Mistake #1: Non-Constant Pointer**

```cpp
const char* kStr = ...;              // pointer itself is mutable
const Thing* const kFoo = ...;       // corrected: pointer is const
constexpr const Thing* kFoo = ...;   // also correct
```

**Mistake #2: String View Without Static Storage**

```cpp
// WRONG: address may change per call
inline absl::string_view MyString() {
  return "Hello";
}
...
Prefer `inline constexpr` variables in headers (C++17+), `extern const` declarations with `constexpr` definitions in source files, `constexpr` functions for compile-time constants, and function-local `static` variables for complex initialization.

---

## TotW #148: Overload Sets

*Originally posted as TotW #148 on May 3, 2018, by Titus Winters. Updated 2020-04-06.*

Source: <https://abseil.io/tips/148>

### Core Principle

The Google C++ style guide emphasizes: "Use overloaded functions only if a reader looking at a call site can understand what is happening without determining which overload is being called."

### What is an Overload Set?

An overload set comprises functions sharing the same name but differing in parameter count, types, or qualifiers. Functions cannot overload solely on return type — the compiler must identify the correct function based only on the invocation's parameters.

### Key Design Guidelines

**Identical Semantics Requirement**: Members of an overload set must have identical documented behavior. If semantics vary between overloads, users must know which function executes, violating the core principle.

**String-ish Parameters**: Forwarding from `std::string` to `const char*` works well because both present identical semantics — accepting string-like data.

**StrCat()**: Demonstrates effective overload set design. This function conceptually remains one entity despite accepting varying parameter counts, avoiding redundant naming schemes.

**Parameter Sinks**: Methods like `std::vector::push_back()` overload on `const T&` and `T&&` to optimize temporary handling while maintaining identical user-facing semantics.

**Overloaded Accessors**: Const/non-const overloads (like `vector::operator[]`) elegantly forward constness without requiring users to understand implementation details.

**Copy vs. Move Constructors**: These form proper overload sets when move operations function as copy optimizations, making the specific invocation irrelevant to users.

### Conclusion

Effective overload sets provide powerful API design frameworks. Misused overloads — where distinguishing between members matters — create confusion and poor maintainability. Success requires ensuring all overload set members deliver consistent, predictable behavior.

---

## TotW #176: Prefer Return Values to Output Parameters

Source: <https://abseil.io/tips/176>

### The Problem with Output Parameters

```cpp
bool ExtractSpecs(Doodad doodad, FooSpec* foo_spec, BarSpec* bar_spec);
```

This pattern creates ambiguity around several critical questions: whether parameters are input-only or input/output, how pre-existing data is handled, whether null pointers are acceptable, what lifetime requirements exist, and what state the output variables have if the function fails.

### The Recommended Solution

Return a struct wrapped in `std::optional`:
...
This approach eliminates ambiguity: inputs and outputs are obvious, objects are created fresh by the function, no null pointers exist, lifetime management is straightforward, and failure states cannot leave objects in undefined states.

### Key Benefits

- Clearer semantic intent.
- Better composability in expressions.
- Reduced cognitive load.
- Compiler-enforceable contracts.
- Easier to use correctly.

### Caveats

Three situations where this approach has limitations: in-out parameters, caller-customized object creation, and potential performance differences. The guidance emphasizes choosing the most sensible API first and only optimizing if evidence supports it.

### Recommendations

Prefer return values to output parameters, use generic wrappers like `std::optional`, and employ structs (not `std::pair` or `std::tuple`) for multiple return values.
