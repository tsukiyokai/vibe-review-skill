...
### 规则 1.1 shape维度乘法必须使用int64_t

严重等级: P0

缺陷描述: shape维度值或元素总数使用int32_t/uint32_t存储，当维度乘积超过2^31-1时溢出。大shape场景(大batch x 大sequence)下buffer分配不足或计算错误。排序索引硬编码DT_INT32、buffer偏移计算、batch维度累乘均为高发场景。
...
### 规则 2.1 tiling侧buffer计算与kernel侧InitBuffer不一致
...
### 规则 2.2 tiling参数未区分normal核和tail核
...
### 规则 2.3 TilingData结构体host-kernel不一致
...
### 规则 2.4 workspace计算公式中错误应用双buffer系数
...
### 规则 2.5 tiling key参数传递错误与dead code
...
### 规则 3.3 多核任务分配未区分tail core和idle core
...
### 规则 4.4 可选输出tensor(outputMask控制)访问前必须检查null
...
### 规则 4.5 禁止在op_host/*.cpp中使用非const全局容器
...
### 规则 5.1 PipeBarrier与FreeTensor时序颠倒
...
### 规则 5.2 SetFlag/WaitFlag必须成对出现且无条件执行
...
### 规则 5.4 连续matmul操作间必须插入PipeBarrier
...
### 规则 7.1 StorageShape/ViewShape混淆
...
### 规则 7.6 TilingParse注册类型与实际CompileInfo类型不一致
...
### 规则 9.1 ascendc_config.json重复/冲突配置
...
### 规则 9.3 新平台binary/compute_units配置遗漏
...
### 规则 9.6 output paramType(REQUIRED/OPTIONAL)配置错误
...
### 规则 10.1 `__CCE_AICORE__`精确匹配应改为范围比较
...
### 规则 13.4 移除binary.json算子变体前确认无下游调用
...
### 规则 15.1 禁止注释EXPECT_*/ASSERT_*断言
