...
##### 规则 4.1.1 shape维度算术运算和GM偏移量必须使用int64_t/uint64_t
...
##### 规则 4.1.2 DataCopy的stride/blockLen参数须校验不超过uint16上限
...
##### 规则 4.1.3 uint无符号减法前必须判断被减数不小于减数
...
- 字节数/元素数不得混淆: 确认API期望的单位(字节 vs 元素)与传入值一致

##### 规则 4.1.5 宽类型向窄类型赋值须显式检查值域
...
##### 规则 4.2.1 新增枚举值/Layout/模式时须全局搜索确认所有分支覆盖

典型缺陷表现: 新增NTD layout后，多处if/switch/映射表未覆盖新值，导致运行时走入错误分支或未定义行为。

审查要点:
- 新增枚举值后，全局搜索该枚举类型的所有switch/if-else链/映射表，逐一确认覆盖
- CANN配置空间维度: Layout(BNSD/BSND/TND/NTD) x 模式(GQA/MHA/MQA) x 量化(FP8/INT8/Perblock) x 平台(910B/910_93/910_95)
- switch语句的default分支应包含错误处理(日志+返回错误)而非静默忽略

##### 规则 4.2.2 多枚举值否定校验中禁止用`||`连接`!=`
...
##### 规则 4.2.3 CHECK/ASSERT宏的条件参数语义方向须与宏定义一致
...
##### 规则 4.3.1 DataCopy前后须有正确方向的流水线同步
...
##### 规则 4.3.2 SetFlag/WaitFlag必须成对出现且无条件执行
...
##### 规则 4.3.3 HardEvent模板参数方向须与数据流一致
...
##### 规则 4.4.1 workspace/buffer大小计算的tiling侧和kernel侧须交叉比对
...
##### 规则 4.4.2 TilingData结构体须使用共享头文件定义
...
##### 规则 4.4.3 tilingKey修改须同时包含host和kernel两侧的diff
...
##### 规则 4.5.1 新增代码与相邻已有代码高度相似时须逐行比对变量名
...
##### 规则 4.5.2 CMake中的OP_NAME须与实际算子目录名一致
...
##### 规则 4.7.1 C++类的内置类型成员须有in-class initializer
...
##### 规则 4.7.2 指针解引用(含作为函数参数)须在nullptr检查之后
...
##### 规则 4.7.3 OP_CHECK宏的返回参数须包含return语句
...
##### 规则 4.8.1 [P0] 并发数据发布须有memory fence
...
##### 规则 4.8.2 [P0] 并发的delete/destroy操作须加锁保护
...
##### 规则 4.8.3 [P0] 协议OpCode/数据结构修改须保持版本兼容
...
##### 规则 4.8.5 [P1] 单例/引用计数初始化须区分首次与后续调用
...
##### 规则 4.9.3 [P1] CeilDiv与CeilAlign语义禁止混淆
...
##### 规则 4.9.4 [P1] StorageShape与ViewShape禁止混淆
...
##### 规则 4.9.5 [P1] 对齐计算须保证方向正确且满足硬件约束
...
##### 规则 4.10.1 单次合入超过3000行或20个文件的PR须拆分
...
##### 规则 4.10.3 接口/API变更须全量搜索调用点并同步修改
